...
if "add_dll_directory" in dir(os):
    ...
    if os.getenv("OPENVDB_TEST_PYTHON_AX"):
        os.add_dll_directory(
            os.getcwd() + "\\..\\..\\..\\..\\openvdb_ax\\openvdb_ax\\" + config
        )
...
def ax_is_enabled():
    ...
    if ax_is_enabled and not ax_hook_exists:
        raise RuntimeError(
            "Expected to test the AX python hooks but "
...
class TestOpenVDB(unittest.TestCase):
    ...
    def testTransform(self):
        xform1 = openvdb.createLinearTransform(
            [[0.5, 0, 0, 0], [0, 1, 0, 0], [0, 0, 2, 0], [1, 2, 3, 1]]
        )
        ...
        self.assertEqual(xform2, xform1)
        xform2 = openvdb.createFrustumTransform(
            taper=0.5,
            depth=100,
            xyzMin=(0, 0, 0),
            xyzMax=(100, 100, 100),
    ...
    def testValueAccessor(self):
        ...
        for factory in openvdb.GridTypes:
            ...
            if grid.valueTypeName != "bool":
                self.assertRaises(
                    TypeError, lambda: acc.setValueOn((5, 5, 5), object())
                )
                self.assertRaises(
                    TypeError, lambda: acc.setValueOff((5, 5, 5), object())
                )
    ...
    def testPickle(self):
        ...
        testXforms = [
            ...
            openvdb.createLinearTransform(
                matrix=[[1, 0, 0, 0], [0, 2, 0, 0], [0, 0, 3, 0], [4, 3, 2, 1]]
            ),
            openvdb.createFrustumTransform(
                (0, 0, 0), (10, 10, 10), taper=0.8, depth=10.0
            ),
        ]
        ...
        for factory in openvdb.GridTypes:
            ...
            for restored, original in zip(
                restoredGrid.iterOnValues(), grid.iterOnValues()
            ):
                self.assertEqual(restored, original)
            # Verify that the original and unpickled grids have the same inactive values.
            for restored, original in zip(
                restoredGrid.iterOffValues(), grid.iterOffValues()
            ):
                self.assertEqual(restored, original)
    ...
    def testGridCombine(self):
        ...
        for original, combined in zip(
            copyOfAGrid.iterOffValues(), aGrid.iterOffValues()
        ):
            self.assertEqual(combined.min, original.min)
            self.assertEqual(combined.max, original.max)
            self.assertEqual(combined.depth, original.depth)
    ...
    def testLevelSetSphere(self):
        ...
        sphere = openvdb.createLevelSetSphere(
            halfWidth=HALF_WIDTH, voxelSize=1.0, radius=100.0
        )
    ...
    def testMeshConversion(self):
        ...
        grids = []
        for gridType in [
            n for n in openvdb.GridTypes if n.__name__ in ("FloatGrid", "DoubleGrid")
        ]:
        ...
        self.assertRaises(
            TypeError,
            lambda: openvdb.BoolGrid.createLevelSetFromPolygons(
                cubePoints, quads=cubeQuads, transform=xform, halfWidth=halfWidth
            ),
        )
        ...
        self.assertRaises(
            TypeError,
            lambda: openvdb.Vec3SGrid.createLevelSetFromPolygons(
                cubePoints, quads=cubeQuads, transform=xform, halfWidth=halfWidth
            ),
        )
        # The "points" argument to createLevelSetFromPolygons() can be a regular array.
        openvdb.FloatGrid.createLevelSetFromPolygons(
        ...
        self.assertRaises(
            TypeError,
            lambda: openvdb.FloatGrid.createLevelSetFromPolygons(
