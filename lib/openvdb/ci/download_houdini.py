...
def service(
    ...
    return _Service(endpoint_url, access_token, access_token_expiry_time)


class _Service(object):
    def __init__(self, endpoint_url, access_token, access_token_expiry_time):
...
def get_access_token_and_expiry_time(access_token_url, client_id, client_secret_key):
...
def call_api_with_access_token(endpoint_url, access_token, function_name, args, kwargs):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "version", type=str, help="Major.Minor version of Houdini to download"
    )
    parser.add_argument("platform", type=str, help="Platform target")
    parser.add_argument(
        "--prod", action="store_true", help="Only download production builds"
    )
    parser.add_argument(
        "--list", action="store_true", help="Just list the available builds and exit."
    )
    ...
    if not re.match("[0-9][0-9]\.[0-9]$", version):
        raise IOError(
            'Invalid Houdini Version "%s", expecting in the form "major.minor" such as "16.0"'
            % version
        )
