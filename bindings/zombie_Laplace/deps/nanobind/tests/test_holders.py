...
def test05a_uniqueptr_from_cpp(clean):
    ...
    with pytest.warns(
        RuntimeWarning,
        match="nanobind: attempted to access a relinquished instance of type 'test_holders_ext.Example'!",
    ):
...
def test05c_uniqueptr_structure_duplicate(clean):
    ...
    with pytest.warns(
        RuntimeWarning, match=r"nanobind::detail::nb_relinquish_ownership()"
    ):
