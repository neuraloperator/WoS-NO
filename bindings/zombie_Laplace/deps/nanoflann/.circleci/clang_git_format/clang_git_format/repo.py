...
class Repo(object):
    ...
    def __init__(self, path, custom_regex="", dirs_in=[], dirs_out=[]):
        ...
        assert isinstance(
            dirs_in, list
        ) and "dirs_in should be a list of directories. Instead got {}".format(dirs_in)
        self.dirs_in = dirs_in
        assert isinstance(
            dirs_out, list
        ) and "dirs_out should be a list of directories. Instead got {}".format(
            dirs_out
        )
    ...
    def langs_used(self, langs_in):
        ...
        if set([i for i in langs_in if i in self.langs_to_file_endings.keys()]) != set(
            langs_in
        ):
            logger.fatal(
                "The following languages are available to use: %s",
    ...
    def filter_files_by_dir(self, file_list):
        ...
        if self.dirs_in:
            for line in file_list:
                if any(
                    [
                        self._dir_filter(d.encode(), line, do_include=True)
        ...
        if self.dirs_out:
            for line in valid_files_in:
                if all(
                    [
                        self._dir_filter(d.encode(), line, do_include=False)
    ...
    def get_files_regexp(self):
        ...
        files_regexp = re.compile("{}\\.{}$".format(self.custom_regex, files_match_str))
