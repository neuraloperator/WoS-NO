...
class ClangFormat:
    ...
    def __init__(self, clang_path, cache_dir):
        ...
        if self.clang_path is None:
            ...
            self.clang_path = os.path.join(
                cache_dir, PROGNAME + self.clang_format_progname_ext
            )

            # Download a new version if the cache is empty or stale
            if not os.path.isfile(self.clang_path) or not self._validate_version():

                logger.error("Haven't found a valid %s version in PATH. ", PROGNAME)
                sys.exit(1)
    ...
    def _lint(self, file_name, print_diff):
        ...
        formatted_file = callo([self.clang_path, "--style=file", file_name]).decode(
            "utf-8"
        )
