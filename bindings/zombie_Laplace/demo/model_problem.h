...
public:
    // constructor
    ModelProblem<T, DIM>(const json& config, std::string directoryPath,std::vector<std::vector<float>>image, std::vector<std::vector<float>>sourceIm, bool isrec, int startx, int starty, int endx, int endy);
    using Int32   = int32_t;
    ...
    std::vector<std::vector<float>> sourceValue;
	std::vector<std::vector<float>> sourceImage;
...
// Implementation
template <typename T, size_t DIM>
ModelProblem<T, DIM>::ModelProblem(const json& config, std::string directoryPath, std::vector<std::vector<float>>image, std::vector<std::vector<float>>sourceIm, bool isrec, int startx, int starty, int endx, int endy):
sdfGridForAbsorbingBoundary(nullptr)
{
	...
	this->h = sourceImage.size();
	this->w = sourceImage[0].size();
...
void ModelProblem<T, DIM>::setupPDE()
{
    ...
    pde.source = [this, bMin, maxLength](const VectorD& x) -> float {
            int x_coord, y_coord;
			x_coord = std::round(x[0]*(this->w));
			y_coord = std::round((x[1])*(this->h));
			//std::cout<<"X COORD: "<<x_coord<< " Y COORD: "<<y_coord<<std::endl;
			x_coord = std::clamp(x_coord, this->x_start, this->x_end);
			y_coord = std::clamp(y_coord, this->y_start, this->y_end);
			//std::cout<<"Inside Second recursion source"<<std::endl;
			//std::cout<<"X COORD: "<<x_coord<< " Y COORD: "<<y_coord<<std::endl;
			
			return sourceValue[y_coord][x_coord];
    };
    pde.dirichlet = [this, bMin, maxLength](const VectorD& x, bool _) -> float {
        if(!this->is_recursive){
            ...
            x_coord = std::clamp(x_coord, this->x_start, this->x_end);
            y_coord = std::clamp(y_coord, this->y_start, this->y_end);
            //std::cout<<" x, y: "<<x[0]<<" "<<x[1] <<" x_c, y_c: "<<x_coord<<" "<<y_coord<<std::endl;
            lap = laplacian(sourceImage, x[0], x[1]);
            ...
            //std::cout<<"Getting Dirichlet Condition"<<std::endl;
            //std::cout<<sourceImage[y_coord][x_coord]<<std::endl;
            return sourceImage[y_coord][x_coord];
        }
    };
