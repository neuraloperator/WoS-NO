...
public:
	...
	Scene(const json &config, std::vector<float>mu1, std::vector<float>mu2, std::vector<float>beta, std::vector<float>r):
	...
	Scene(const json &config, std::vector<float>mu1, std::vector<float>mu2, std::vector<float>beta, std::vector<float>r, std::vector<std::string>geometry):
	...
	Scene(const json &config, std::vector<std::vector<float>> sourceValue_mat, std::vector<float>mu1, std::vector<float>mu2, std::vector<float>beta, std::vector<float>r):
		  isWatertight(getOptional<bool>(config, "isWatertight", false)),
		  isDoubleSided(getOptional<bool>(config, "isDoubleSided", false)),
		  queries(isWatertight)
	{
...
private:
	...
	void separateBoundaries() {
		...
		for (int i = 0; i < segments.size(); i++) {
			Vector2 pMid = 0.5f * (vertices[segments[i][0]] + vertices[segments[i][1]]); //Find the mid-point of the edges
