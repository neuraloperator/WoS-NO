...
def orient_curves_by_containment_order(path_v, path_l, path_props):
    ...
    signed_area = np.array(
        [
            -polygon[i].area if ring[i].is_ccw else polygon[i].area
            for i in range(n_paths)
        ]
    )
    ...
    contains = np.array(
        [
            [
                (1 if polygon[i].contains_properly(ring[j]) else 0) if closed[i] else 0
    ...
    for idx in curves_largest_to_smallest_area:
        ...
        if closed[idx] and len(containing_curves) > 0:
            ...
            parent_idx = containing_curves[
                np.argsort(signed_area[containing_curves])[0]
            ]
            flip_orientation[idx] = np.sign(signed_area[parent_idx]) == np.sign(
                signed_area[idx]
            )
...
def svg_to_obj(
    svg_path, obj_path, normalize, use_ccw, auto_orient_curves, n_points_per_segment
):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--out", type=str, nargs="?", help="Path to the output OBJ file"
    )
    parser.add_argument(
        "--normalize", action="store_true", help="Normalize vertex positions"
    )
    parser.add_argument(
        "--use_ccw",
        action="store_true",
        help="Use counter-clockwise orientation instead of clockwise",
    )
    parser.add_argument(
        "--auto_orient_curves",
        action="store_true",
        help="Orient curves so that they are coherent. Only for non-overlapping, closed curves.",
    )
    parser.add_argument(
        "--n_points_per_segment",
        type=int,
        nargs="?",
        help="Number of points along each segment of an SVG path.",
        default=10,
    )
    ...
    output = (
        os.path.splitext(args.out if args.out is not None else args.svg_filename)[0]
        + ".obj"
    )
