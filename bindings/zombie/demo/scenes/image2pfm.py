...
def image_to_pfm(
    image_path, pfm_path, normalize=False, flip_y=False, scale=1.0, offset=0.0
):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "images", type=str, nargs="+", help="List of input files to convert"
    )
    parser.add_argument(
        "--normalize",
        action="store_true",
        help="Map values to 0 to 1 range by dividing 255",
    )
    parser.add_argument(
        "--flip_y",
        action="store_true",
        help="Flip PFM along vertical direction before saving",
    )
    ...
    for image in images:
        ...
        image_to_pfm(
            image, output, args.normalize, args.flip_y, args.scale, args.offset
        )
