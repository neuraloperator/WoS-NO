...
void createSolutionGrid(std::vector<zombie::SamplePoint<float, 3>>& samplePts,
...
void createSolutionGrid_3d(std::vector<zombie::SamplePoint<float, 3>>& samplePts,
	...
	for (int i = 0; i < scale1 * gridRes; i++) {
		for (int j = 0; j < scale2 * gridRes; j++) {
			for (int k = 0; k < scale3 * gridRes; k++) {
				Vector3 pt((i / float(scale1 * gridRes)) * extent.x() + bMin.x(),
						(j / float(scale2 * gridRes)) * extent.y() + bMin.y(),
						(k / float(scale3 * gridRes)) * extent.z() + bMin.z());
				std::vector<float> temp {pt[0], pt[1], pt[3]};
				pts.emplace_back(temp);
				float dDist = queries.computeDistToDirichlet(pt, false);
				float nDist = queries.computeDistToNeumann(pt, false);
				samplePts.emplace_back(zombie::SamplePoint<float, 3>(pt, Vector3::Zero(),
...
std::vector<float> getSolution(const std::vector<zombie::SamplePoint<float, 3>>& samplePts,
...
std::vector<float> getSolution(const std::vector<zombie::SamplePoint<float, 3>>& samplePts,
								const zombie::PDEVar<float, 3>& pde,
								const zombie::GeometricQueries<3> &queries,
								const bool isDoubleSided, const json &config) {
	const float boundaryDistanceMask = getOptional<float>(config, "boundaryDistanceMask", 0.0);
...
std::vector<float> getSolution_bvc(const std::vector<zombie::EvaluationPoint<float, 2>>& samplePts,
								const zombie::PDE<float, 2>& pde,
								const zombie::GeometricQueries<2> &queries,
								const bool isDoubleSided, const json &config) {
	const float boundaryDistanceMask = getOptional<float>(config, "boundaryDistanceMask", 0.0);
	...
	for (int idx = 0; idx < samplePts.size(); idx++) {
		...
		// solution data
		//float value = samplePts[idx].statistics ? samplePts[idx].statistics->getEstimatedSolution(): 0.0f;
		float value = samplePts[idx].getEstimatedSolution();
		// bool maskOutValue = (!inDomain && !isDoubleSided) ||
		// 					std::abs(neumannDist) < boundaryDistanceMask;
		bool maskOutValue = std::abs(neumannDist) < boundaryDistanceMask;
		//solution[idx] = maskOutValue ? 0.0f : value;
...
std::vector<float> getSolution_bvc(const std::vector<zombie::EvaluationPoint<float, 3>>& samplePts,
	const zombie::PDEVar<float, 3>& pde,
	const zombie::GeometricQueries<3> &queries,
	const bool isDoubleSided, const json &config) {
const float boundaryDistanceMask = getOptional<float>(config, "boundaryDistanceMask", 0.0);
...
std::vector<std::vector<float>> getGradient(const std::vector<zombie::SamplePoint<float, 3>>& samplePts,
								const zombie::PDE<float, 3>& pde,
								const zombie::GeometricQueries<3> &queries,
								const bool isDoubleSided, const json &config) {
	const float boundaryDistanceMask = getOptional<float>(config, "boundaryDistanceMask", 0.0);
...
std::vector<std::vector<float>> getGradient_bvc(const std::vector<zombie::EvaluationPoint<float, 2>>& samplePts,
								const zombie::PDE<float, 2>& pde,
								const zombie::GeometricQueries<2> &queries,
								const bool isDoubleSided, const json &config) {
	const float boundaryDistanceMask = getOptional<float>(config, "boundaryDistanceMask", 0.0);
	...
	for (int idx = 0; idx < samplePts.size(); idx++) {
		...
		solution[idx][0] = maskOutValue ? 0.0f : value;
		solution[idx][1] = maskOutValue ? 0.0f : value;
		// solution[idx][0] = value[0];
		// solution[idx][1] = value[1];
	}
...
std::vector<std::vector<float>> getGradient_bvc(const std::vector<zombie::EvaluationPoint<float, 3>>& samplePts,
	const zombie::PDEVar<float, 3>& pde,
	const zombie::GeometricQueries<3> &queries,
	const bool isDoubleSided, const json &config) {
	const float boundaryDistanceMask = getOptional<float>(config, "boundaryDistanceMask", 0.0);
	...
	solution[idx][0] = maskOutValue ? 0.0f : value;
	solution[idx][1] = maskOutValue ? 0.0f : value;
	solution[idx][2] = maskOutValue ? 0.0f : value;
	// solution[idx][0] = value[0];
	// solution[idx][1] = value[1];
	}
...
void createEvaluationGrid(std::vector<zombie::EvaluationPoint<float, 2>>& evalPts,
						  const zombie::GeometricQueries<2> &queries,
						  const Vector2 &bMin, const Vector2 &bMax,const int gridRes) {
	Vector2 extent = bMax - bMin;
	for (int i = 0; i < gridRes; i++) {
		for (int j = 0; j < gridRes; j++) {
			Vector2 pt((i / float(gridRes)) * extent.x() + bMin.x(),
...
void createEvaluationGrid(std::vector<zombie::EvaluationPoint<float, 2>>& evalPts,
						  const zombie::GeometricQueries<2> &queries,
						  const Vector2 &bMin, const Vector2 &bMax,
						  const int gridRes, std::vector<std::vector<float>>& pts) {
	// Vector2 extent = bMax - bMin;
	// for (int i = 0; i < gridRes; i++) {
	// 	for (int j = 0; j < gridRes; j++) {
	// 		Vector2 pt((i / float(gridRes)) * extent.x() + bMin.x(),
...
void createEvaluationGrid(std::vector<zombie::EvaluationPoint<float, 3>>& evalPts,
	const zombie::GeometricQueries<3> &queries,
	const Vector3 &bMin, const Vector3 &bMax,
	const int gridRes, std::vector<std::vector<float>>& pts) {
// Vector2 extent = bMax - bMin;
...
for (int i = 0; i < pts.size(); ++i) {
	...
	float dDist = queries.computeDistToDirichlet(pt, false);
	float nDist = queries.computeDistToNeumann(pt, false);
	evalPts.emplace_back(zombie::EvaluationPoint<float, 3>(pt, Vector3::Zero(),
											zombie::SampleType::InDomain,
											dDist, nDist, 0.0f));
}
}
