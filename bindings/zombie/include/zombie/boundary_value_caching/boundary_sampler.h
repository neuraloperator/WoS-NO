...
template <>
inline Vector3 lineSegmentNormal<3>(const Vector3& pa, const Vector3& pb, bool normalize)
{
	Vector3 s = pb - pa;
	Vector3 n(s[2], -s[1], s[0]); // 2D normal in 3D space
...
template <>
inline float lineSegmentSurfaceArea<3>(const Vector3& pa, const Vector3& pb)
{
	return lineSegmentNormal<3>(pa, pb, false).norm();
}
...
public:
	...
	void initialize(float normalOffsetForDirichletSamples, bool solveDoubleSided,
		...
		// compute normals
		std::cout<<"Computing boundary normals..."<<std::endl;
		computeNormals(computeWeightedNormals);
		std::cout<<"Computed boundary normals."<<std::endl;
		...
		std::cout<<"Built CDF table for Dirichlet boundary."<<std::endl;
		...
		if (solveDoubleSided) {
			...
			std::cout<<"Built CDF table for Dirichlet boundary (normal aligned)." << std::endl;
		}
	}
	...
	// solves the given PDE at the generated sample points
	void computeEstimates(const PDEVar<T, DIM>& pde, const WalkSettings<T>& walkSettings,
		int nWalksForSolutionEstimates, int nWalksForGradientEstimates,
		std::vector<SamplePoint<T, DIM>>& samplePts,
		bool useFiniteDifferences=false, bool runSingleThreaded=false,
		std::function<void(int,int)> reportProgress={}) const {
