...
public:
	...
	FcpwSceneLoader(const std::vector<Vector3>& positions,
		...
		//std::cout<<"3D FCPW SCENE LOADER"<<std::endl;
		if (positions.size() > 0) {
			// scene geometry is made up of triangles
			std::vector<std::vector<fcpw::PrimitiveType>> objectTypes(
				1, std::vector<fcpw::PrimitiveType>{fcpw::PrimitiveType::LineSegment}); //CHANGE FROM Triangle TO LINE SEGMENT FOR WIRE MESH
			scene.setObjectTypes(objectTypes);
			...
			scene.setObjectLineSegmentCount(T, 0);
			// specify the vertex positions
			for (int i = 0; i < V; i++) {
				scene.setObjectVertex(positions[i], i, 0);
			}
			//std::cout<<"OBJECT VERTEX POSITIONS SET"<<std::endl;
			// // specify the triangle indices
			// for (int i = 0; i < T; i++) {
			// 	int index[3] = {(int)indices[i][0], (int)indices[i][1], (int)indices[i][2]};
			// 	std::cout<<"INDICES: "<<indices[i][0]<<" "<<indices[i][1]<<" "<<indices[i][2]<<std::endl;
			// 	scene.setObjectTriangle(index, i, 0);
			// }

			// specify the line segment indices  3D WIRE MESH WITHOUT FACES
			for (int i = 0; i < T; i++) {
				//std::cout<<"OBJ LINE SEGMENT SETTING"<<std::endl;
				int index[2] = {(int)indices[i][0], (int)indices[i][1]};
				//std::cout<<"INDICES: "<<indices[i][0]<<" "<<indices[i][1]<<std::endl;
				scene.setObjectLineSegment(index, i, 0);
				//std::cout<<"OBJ LINE SEGMENT SET"<<std::endl;
			}

			//std::cout<<"OBJECT TRIANGLES SET"<<std::endl;
			...
			//std::cout<<"Computed SILHOUETTES"<<std::endl;
			...
			fcpw::AggregateType aggregateType = buildBvh ?
												...
												fcpw::AggregateType::Baseline;
			scene.build(aggregateType, true, true, true);
			//std::cout<<"BUILT AGGREGATE"<<std::endl;
		}
	}
