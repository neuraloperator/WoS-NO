...
public:
	...
	// solves the given variational PDE at the input point; NOTE: assumes the point does not
	// lie on the boundary when estimating the gradient
	...
	// solves the given variational PDE at the input points (in parallel by default); NOTE:
	// assumes points do not lie on the boundary when estimating gradients
	void solve(const PDEVar<T, DIM>& pde,
			   const WalkSettings<T>& walkSettings,
			   const std::vector<SampleEstimationData<DIM>>& estimationData,
...
private:
	...
	// performs a single reflecting random walk starting at the input point for Varying PDE
	WalkCompletionCode walk(const PDEVar<T, DIM>& pde,
		...
		while (dirichletDist > walkSettings.epsilonShell) {
			...
			greensFn->updateBall(state.currentPt, dirichletDist);
			...
			float diffusionWeight = 1.0f/std::sqrt(diffusion*pde.diffusion(greensFn->yVol));
			T sourceContribution = greensFn->norm()*pde.source(greensFn->yVol)*diffusionWeight;
			...
			if (!intersectedNeumann && queries.outsideBoundingDomain(intersectionPt.pt)) {
			...
			bool sampleVolumeIntegral = sampleUniform() < pde.bound*greensFnNorm;
			if (sampleVolumeIntegral) {
				...
				float pdf;
				state.currentPt = greensFn->sampleVolume(direction, sampler, sourcePdf);
				...
				float pdf;
				state.currentPt = greensFn->sampleSurface(sampler, pdf);
			}
			...
			// if (pde.mediumType != MediumType::Empty && walkSettings.stepsBeforeApplyingTikhonov == state.walkLength)  {
			// 	//greensFn = std::make_unique<YukawaGreensFnBall<DIM>>(pde.bound);
	...
	void setTerminalContribution(WalkCompletionCode code, const PDEVar<T, DIM>& pde,
								 const WalkSettings<T>& walkSettings,
								 WalkState<T, DIM>& state) const {
		if (code == WalkCompletionCode::ReachedDirichletBoundary && !walkSettings.ignoreDirichletContribution) {
			// project the walk position to the Dirichlet boundary and grab the known boundary value
	...
	void estimateSolution(const PDEVar<T, DIM>& pde,
		...
		if (samplePt.type == SampleType::OnDirichletBoundary) {
			if (!hasPrevEstimates) {
				...
				if (!walkSettings.ignoreDirichletContribution) {
					totalContribution = pde.dirichlet(samplePt.pt);
				}
		...
		for (int w = 0; w < nWalks; w++) {
			...
			if (pde.mediumType == MediumType::Empty) { 
				//greensFn = std::make_unique<HarmonicGreensFnBall<DIM>>();
				greensFn = std::make_unique<YukawaGreensFnBall<DIM>>(pde.bound);
			...
			if ((code == WalkCompletionCode::ReachedDirichletBoundary ||
				...
				T totalContribution = state.throughput*state.terminalContribution + state.totalSourceContribution;
