...
def _test(arg=None):
    ...
    if platform.system() == "Linux":
        ...
        assert 256 == os.system(
            "ldd " + _api.__file__ + "| grep -E 'libimf|libsvml|libintlc'"
        )  # nosec
...
class TBBProcessPool27(multiprocessing.pool.Pool):
    def _repopulate_pool(self):
        ...
        for i in range(self._processes - len(self._pool)):
            w = self.Process(
                target=tbb_process_pool_worker27,
                args=(
                    self._inqueue,
                    self._outqueue,
                    self._initializer,
...
class TBBProcessPool3(multiprocessing.pool.Pool):
    def _repopulate_pool(self):
        ...
        for i in range(self._processes - len(self._pool)):
            w = self.Process(
                target=tbb_process_pool_worker3,
                args=(
                    self._inqueue,
                    self._outqueue,
                    self._initializer,
...
class Monkey:
    ...
    def __init__(self, max_num_threads=None, benchmark=False):
        ...
        """
        if max_num_threads:
            self.ctl = global_control(
                global_control.max_allowed_parallelism, int(max_num_threads)
            )
        if benchmark:
    ...
    def _patch(self, class_name, module_name, obj):
        m = self._modules[class_name] = __import__(
...
def init_sem_name():
    try:
        ...
        print(
            "Warning: Can not initialize name of shared semaphores:", e, file=sys.stderr
        )
...
def _main():
    ...
    parser.add_argument(
        ...
        type=int,
        help="Initialize oneTBB with P max number of threads per process",
        metavar="P",
    )
    parser.add_argument(
        "-b",
        ...
        if args.allocator and not os.environ.get("_TBB_MALLOC_PRELOAD"):
            ...
            if libtbbmalloc_lib in preload_list:
                ...
                os.environ[ld_preload] = ":".join(
                    [libtbbmalloc_lib] + list(preload_list)
                )
            ...
            if args.allocator_huge_pages:
                ...
                try:
                    ...
                    if pages == 0:
                        print(
                            "oneTBB: Pre-allocated huge pages are not currently reserved in the system. To reserve, run e.g.:\n"
                            "\tsudo sh -c 'echo 2000 > /proc/sys/vm/nr_hugepages'"
                        )
                    os.environ["TBB_MALLOC_USE_HUGE_PAGES"] = "1"
                except:
                    print(
                        "oneTBB: Failed to read number of pages from /proc/sys/vm/nr_hugepages\n"
