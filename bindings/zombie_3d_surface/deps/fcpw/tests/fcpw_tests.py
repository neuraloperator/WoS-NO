...
def isolate_interior_points(scene, query_points):
    ...
    interior_points = [
        query_point
        for query_point, interior in zip(query_points, is_interior)
        if interior
    ]
        ...
        for line in file:
            if dim == 2:
                if line.startswith("v "):
                    ...
                    index = [
                        int(idx.split("/")[0]) - 1 for idx in line.strip().split()[1:]
                    ]
                    ...
                    for i in range(F - 1):
                        j = (i + 1) % F
                        indices.append(
                            np.array([index[i], index[j]], dtype=np.int32, order="C")
                        )
...
def run_cpu_ray_intersection_queries(
    scene,
    ray_origins,
    ray_directions,
    ray_distance_bounds,
    ...
    if run_bundled_queries:
        ...
        scene.intersect(
            ray_origins, ray_directions, ray_distance_bounds, interactions, False
        )
        end_time = time.perf_counter()
        print(
            f"{n_queries} ray intersection queries took {end_time - start_time} seconds"
        )
        ...
        if dim == 2:
            for q in range(n_queries):
                ray = fcpw.ray_2D(
                    ray_origins[q], ray_directions[q], ray_distance_bounds[q]
                )
...
def run_cpu_sphere_intersection_queries(
    scene,
    sphere_centers,
    sphere_squared_radii,
    rand_nums,
    dim,
    run_bundled_queries=True,
):
    ...
    if run_bundled_queries:
        ...
        start_time = time.perf_counter()
        scene.intersect(
            sphere_centers, sphere_squared_radii, interactions, rand_nums, None
        )
        end_time = time.perf_counter()
        print(
            f"{n_queries} sphere intersection queries took {end_time - start_time} seconds"
        )
        ...
        if dim == 2:
            for q in range(n_queries):
                sphere = fcpw.bounding_sphere_2D(
                    sphere_centers[q], sphere_squared_radii[q]
                )
                interactions[q] = fcpw.interaction_2D()
            ...
            for q in range(n_queries):
                sphere = fcpw.bounding_sphere_3D(
                    sphere_centers[q], sphere_squared_radii[q]
                )
                interactions[q] = fcpw.interaction_3D()
                hits = scene.intersect(sphere, interactions[q], rand_nums[q], None)
...
def run_cpu_closest_point_queries(
    scene, query_points, squared_max_radii, dim, run_bundled_queries=True
):
    n_queries = len(query_points)
    if run_bundled_queries:
        interactions = None
        ...
        if dim == 2:
            for q in range(n_queries):
                interactions[q] = fcpw.interaction_2D()
                found = scene.find_closest_point(
                    query_points[q], interactions[q], squared_max_radii[q]
                )
...
def run_cpu_closest_silhouette_point_queries(
    scene,
    query_points,
    squared_max_radii,
    ...
    if run_bundled_queries:
        ...
        start_time = time.perf_counter()
        scene.find_closest_silhouette_points(
            query_points, squared_max_radii, interactions, flip_normal_orientation
        )
        end_time = time.perf_counter()
        print(
            f"{n_queries} closest silhouette point queries took {end_time - start_time} seconds"
        )
        ...
        if dim == 2:
            for q in range(n_queries):
                interactions[q] = fcpw.interaction_2D()
                found = scene.find_closest_silhouette_point(
                    query_points[q],
                    interactions[q],
                    flip_normal_orientation[q],
                    squared_max_radii[q],
                )
...
@wp.kernel
def run_warp_ray_intersection_queries(
    mesh: wp.uint64,
...
@wp.kernel
def run_warp_closest_point_queries(
    mesh: wp.uint64,
...
def compare_cpu_interactions(cpu_interactions_baseline, cpu_interactions, dim):
    ...
    for i in range(n_queries):
        ...
        different_indices = (
            cpu_interaction_baseline.primitive_index == -1
            and cpu_interaction.primitive_index != -1
        ) or (
            cpu_interaction_baseline.primitive_index != -1
            and cpu_interaction.primitive_index == -1
        )
        ...
        if different_indices:
            ...
            if dim == 2:
                ...
                print(
                    f"\tp: {cpu_interaction_baseline.p[0]} {cpu_interaction_baseline.p[1]}"
                )
                print(
                    f"\tn: {cpu_interaction_baseline.n[0]} {cpu_interaction_baseline.n[1]}"
                )
                ...
                print("CPU Interaction Baseline")
                print(
                    f"\tp: {cpu_interaction_baseline.p[0]} {cpu_interaction_baseline.p[1]} {cpu_interaction_baseline.p[2]}"
                )
                print(
                    f"\tn: {cpu_interaction_baseline.n[0]} {cpu_interaction_baseline.n[1]} {cpu_interaction_baseline.n[2]}"
                )
                print(
                    f"\tuv: {cpu_interaction_baseline.uv[0]} {cpu_interaction_baseline.uv[1]}"
                )
                print(f"\td: {cpu_interaction_baseline.d}")
                print(f"\tindex: {cpu_interaction_baseline.primitive_index}")
                print("CPU Interaction")
                print(
                    f"\tp: {cpu_interaction.p[0]} {cpu_interaction.p[1]} {cpu_interaction.p[2]}"
                )
                print(
                    f"\tn: {cpu_interaction.n[0]} {cpu_interaction.n[1]} {cpu_interaction.n[2]}"
                )
...
def compare_cpu_gpu_interactions(cpu_interactions, gpu_interactions, dim):
    ...
    for i in range(n_queries):
        ...
        different_indices = (
            cpu_interaction.primitive_index == -1
            and gpu_interaction.index != 4294967295
        ) or (
            cpu_interaction.primitive_index != -1
            and gpu_interaction.index == 4294967295
        )
        ...
        if different_indices:
            ...
            if dim == 2:
                ...
                print("GPU Interaction")
                print(
                    f"\tp: {gpu_interaction.p.x} {gpu_interaction.p.y} {gpu_interaction.p.z}"
                )
                print(
                    f"\tn: {gpu_interaction.n.x} {gpu_interaction.n.y} {gpu_interaction.n.z}"
                )
...
def compare_warp_and_gpu_interactions(
    warp_faces, warp_points, warp_dist, gpu_interactions
):
    ...
    for i in range(n_queries):
        ...
        ) or (warp_faces[i] != -1 and gpu_interaction.index == 4294967295)
...
def visualize_polyscope_scene(
    positions,
    indices,
    query_points,
    random_directions,
    random_squared_radii,
    interior_points,
    dim,
):
    ps.init()
    ...
    ps.get_point_cloud("query points").add_vector_quantity(
        "random directions", random_directions
    )
    ps.get_point_cloud("query points").add_scalar_quantity(
        "random squared radii", random_squared_radii
    )
...
def run(
    ...
    print("\nBuilding BVH on CPU")
    scene = load_fcpw_scene(
        positions,
        indices,
        fcpw.aggregate_type.bvh_overlap_surface_area,
        compute_silhouettes,
        False,
        dim,
        True,
    ...
    is_interior = tag_interior_points(scene, query_points)
    flip_normal_orientation = np.array(
        [0 if not is_interior[q] else 1 for q in range(n_queries)], dtype=int
    )
    ...
    if compare_with_cpu_baseline:
        baseline_scene = load_fcpw_scene(
            positions,
            indices,
            fcpw.aggregate_type.baseline,
            compute_silhouettes,
            False,
            dim,
            True,
    ...
    if run_gpu_queries:
        ...
        gpu_scene.intersect(
            query_points, random_directions, query_bounds, gpu_ray_interactions
        )

        gpu_sphere_interactions = fcpw.gpu_interaction_list()
        gpu_scene.intersect(
            query_points, random_squared_radii, rand_nums, gpu_sphere_interactions
        )
        ...
        if compute_silhouettes:
            ...
            gpu_scene.find_closest_silhouette_points(
                query_points,
                query_bounds,
                flip_normal_orientation,
    ...
    if compare_with_warp and run_gpu_queries and dim == 3:
        ...
        device_directions = wp.array(
            data=random_directions, dtype=wp.vec3, device="cuda:0"
        )
        device_parametric_dist = wp.full(
            shape=n_queries, value=np.inf, dtype=float, device="cuda:0"
        )
        wp_mesh = wp.Mesh(
            points=wp.array(data=positions, dtype=wp.vec3, device="cuda:0"),
            indices=wp.array(data=indices.flatten(), dtype=int, device="cuda:0"),
            velocities=None,
            bvh_constructor="sah",
        )

        host_intersection_faces = wp.full(
            shape=n_queries, value=-1, dtype=int, device="cpu"
        )
        host_intersection_hit_points = wp.zeros(
            shape=n_queries, dtype=wp.vec3, device="cpu"
        )
        host_intersection_dist = wp.full(
            shape=n_queries, value=np.inf, dtype=float, device="cpu"
        )
        device_intersection_faces = wp.full(
            shape=n_queries, value=-1, dtype=int, device="cuda:0"
        )
        device_intersection_hit_points = wp.zeros(
            shape=n_queries, dtype=wp.vec3, device="cuda:0"
        )
        device_intersection_dist = wp.full(
            shape=n_queries, value=np.inf, dtype=float, device="cuda:0"
        )
        with wp.ScopedTimer(
            "Warp ray intersection queries", cuda_filter=wp.TIMING_KERNEL
        ):
            wp.launch(
                kernel=run_warp_ray_intersection_queries,
                dim=n_queries,
                inputs=[
                    wp_mesh.id,
                    device_points,
                    device_directions,
                    device_parametric_dist,
                    device_intersection_faces,
                    device_intersection_hit_points,
                    device_intersection_dist,
                ],
                device="cuda:0",
            )
        ...
        host_closest_point_faces = wp.full(
            shape=n_queries, value=-1, dtype=int, device="cpu"
        )
        host_closest_points = wp.zeros(shape=n_queries, dtype=wp.vec3, device="cpu")
        host_closest_point_dist = wp.full(
            shape=n_queries, value=np.inf, dtype=float, device="cpu"
        )
        device_closest_point_faces = wp.full(
            shape=n_queries, value=-1, dtype=int, device="cuda:0"
        )
        device_closest_points = wp.zeros(
            shape=n_queries, dtype=wp.vec3, device="cuda:0"
        )
        device_closest_point_dist = wp.full(
            shape=n_queries, value=np.inf, dtype=float, device="cuda:0"
        )
        with wp.ScopedTimer("Warp closest point queries", cuda_filter=wp.TIMING_KERNEL):
            wp.launch(
                kernel=run_warp_closest_point_queries,
                dim=n_queries,
                inputs=[
                    wp_mesh.id,
                    device_points,
                    device_parametric_dist,
                    device_closest_point_faces,
                    device_closest_points,
                    device_closest_point_dist,
                ],
                device="cuda:0",
            )
        ...
        compare_warp_and_gpu_interactions(
            host_intersection_faces.numpy(),
        ...
        compare_warp_and_gpu_interactions(
            host_closest_point_faces.numpy(),
    ...
    if compare_with_cpu_baseline:
        ...
        compare_cpu_interactions(
            baseline_cpu_ray_interactions, cpu_ray_interactions, dim
        )
        print("\nComparing CPU closest point query results...")
        compare_cpu_interactions(
            baseline_cpu_cpq_interactions, cpu_cpq_interactions, dim
        )
        if compute_silhouettes:
            print("\nComparing CPU closest silhouette point query results...")
            compare_cpu_interactions(
                baseline_cpu_cspq_interactions, cpu_cspq_interactions, dim
            )
    ...
    if run_gpu_queries:
        ...
        compare_cpu_gpu_interactions(
            cpu_sphere_interactions, gpu_sphere_interactions, dim
        )
        ...
        if compute_silhouettes:
            ...
            compare_cpu_gpu_interactions(
                cpu_cspq_interactions, gpu_cspq_interactions, dim
            )

    if visualize_scene:
        interior_points = isolate_interior_points(scene, query_points)
        visualize_polyscope_scene(
            positions,
            indices,
            query_points,
            random_directions,
...
def main():
    ...
    parser.add_argument(
        "--file_path", type=str, required=True, help="path to the input OBJ file"
    )
    parser.add_argument("--dim", type=int, required=True, help="scene dimensionality")
    parser.add_argument(
        "--n_queries", type=int, required=True, help="number of queries"
    )
    parser.add_argument(
        "--compute_silhouettes", action="store_true", help="compute silhouettes"
    )
    parser.add_argument(
        "--compare_with_cpu_baseline",
        action="store_true",
        help="compare with CPU baseline",
    )
    parser.add_argument(
        "--run_gpu_queries", action="store_true", help="run GPU queries"
    )
    parser.add_argument(
        "--compare_with_warp", action="store_true", help="compare with warp"
    )
    parser.add_argument(
        "--refit_gpu_scene", action="store_true", help="refit GPU scene"
    )
    parser.add_argument(
        "--visualize_scene", action="store_true", help="visualize scene"
    )
    ...
    run(
        args.file_path,
        args.n_queries,
        args.compute_silhouettes,
        args.compare_with_cpu_baseline,
        args.run_gpu_queries,
        args.compare_with_warp,
        args.refit_gpu_scene,
