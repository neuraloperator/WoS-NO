...
// The estimated PDEVar solution, gradient, and other statistics can be queried
// from the SampleStatistics struct, which is stored in the SamplePoint struct.
// Each call to solve(...) improves the estimate of the PDEVar solution at the
...
public:
    // constructors
    DeltaTrack(const GeometricQueries<DIM>& queries_);
    DeltaTrack(const GeometricQueries<DIM>& queries_,
    ...
    // solves the given PDEVar at the input point; NOTE: assumes the point does not
    // lie on the boundary when estimating the gradient
    void solve(const PDEVar<T, DIM>& pde,
               const WalkSettings& walkSettings,
               int nWalks, SamplePoint<T, DIM>& samplePt) const;

    // solves the given PDEVar at the input points (in parallel by default);
...
protected:
    ...
    void computeReflectingBoundaryContribution(const PDEVar<T, DIM>& pde,
                                               const WalkSettings& walkSettings,
    ...
    void computeSourceContribution(const PDEVar<T, DIM>& pde,
                                   const WalkSettings& walkSettings,
                                   const Vector<DIM>& direction,
                                   float intersectionDist, pcg32& sampler,
                                   WalkState<T, DIM>& state, float diffusion) const;

    // computes the throughput of a single walk step
    float computeWalkStepThroughput(const PDEVar<T, DIM>& pde,
                                    const WalkSettings& walkSettings,
    ...
    WalkCompletionCode walk(const PDEVar<T, DIM>& pde,
                            const WalkSettings& walkSettings,
    ...
    T getTerminalContribution(WalkCompletionCode code,
                              const PDEVar<T, DIM>& pde,
                              const WalkSettings& walkSettings,
                              WalkState<T, DIM>& state) const;

    // estimates only the solution of the given PDEVar at the input point
    void estimateSolution(const PDEVar<T, DIM>& pde,
                          const WalkSettings& walkSettings,
                          int nWalks, SamplePoint<T, DIM>& samplePt) const;

    // estimates the solution and gradient of the given PDEVar at the input point;
    ...
    void estimateSolutionAndGradient(const PDEVar<T, DIM>& pde,
                                     const WalkSettings& walkSettings,
                                     int nWalks, SamplePoint<T, DIM>& samplePt) const;
...
template <typename T, size_t DIM>
inline DeltaTrack<T, DIM>::DeltaTrack(const GeometricQueries<DIM>& queries_):
...
template <typename T, size_t DIM>
inline void DeltaTrack<T, DIM>::solve(const PDEVar<T, DIM>& pde,
                                       const WalkSettings& walkSettings,
                                       int nWalks, SamplePoint<T, DIM>& samplePt) const
{
...
inline void DeltaTrack<T, DIM>::solve(const PDEVar<T, DIM>& pde,
                                       ...
                                       std::function<void(int, int)> reportProgress) const
{
    // solve the PDEVar at each point independently
...
template <typename T, size_t DIM>
inline void DeltaTrack<T, DIM>::computeReflectingBoundaryContribution(const PDEVar<T, DIM>& pde,
                                                                       const WalkSettings& walkSettings,
                                                                       float starRadius, bool flipNormalOrientation,
                                                                       pcg32& sampler, WalkState<T, DIM>& state) const
{
...
template <typename T, size_t DIM>
inline void DeltaTrack<T, DIM>::computeSourceContribution(const PDEVar<T, DIM>& pde,
                                                           const WalkSettings& walkSettings,
                                                           const Vector<DIM>& direction,
                                                           float intersectionDist, pcg32& sampler,
                                                           WalkState<T, DIM>& state, float diffusion) const
{
    if (!walkSettings.ignoreSourceContribution) {
        ...
        float diffusionWeight = 1.0f/std::sqrt(diffusion * pde.diffusion(sourcePt));
        T sourceContribution = state.greensFn->norm()*pde.source(sourcePt)*diffusionWeight;
        state.totalSourceContribution += state.throughput*sourceContribution;
    }
}

template <typename T, size_t DIM>
inline float DeltaTrack<T, DIM>::computeWalkStepThroughput(const PDEVar<T, DIM>& pde,
                                                            const WalkSettings& walkSettings,
                                                            const WalkState<T, DIM>& state) const
{
...
template <typename T, size_t DIM>
inline bool DeltaTrack<T, DIM>::applyWeightWindow(const WalkSettings& walkSettings,
                                                   pcg32& sampler, WalkState<T, DIM>& state,
                                                   std::queue<WalkState<T, DIM>>& stateQueue) const
{
...
template <typename T, size_t DIM>
inline WalkCompletionCode DeltaTrack<T, DIM>::walk(const PDEVar<T, DIM>& pde,
                                                    const WalkSettings& walkSettings,
                                                    float distToAbsorbingBoundary,
                                                    float firstSphereRadius,
                                                    bool flipNormalOrientation,
                                                    ...
                                                    std::queue<WalkState<T, DIM>>& stateQueue) const
{
    ...
    bool firstStep = true;
    float diffusion = pde.diffusion(state.currentPt);
    while (distToAbsorbingBoundary > walkSettings.epsilonShellForAbsorbingBoundary) {
        ...
        computeSourceContribution(pde, walkSettings, direction, intersectionPt.dist, sampler, state, diffusion);

        bool sampleVolumeIntegral = sampleUniform() < pde.bound*state.greensFn->norm();
        if (sampleVolumeIntegral) {
            float sourceRadius, sourcePdf;
            state.currentPt = state.greensFn->sampleVolume(sampler, sourceRadius, sourcePdf);
            state.throughput *= (1.0f - pde.transformedAbsorption(state.currentPt)/pde.bound);
        }
        else{
            float sourceRadius, sourcePdf;
            state.currentPt = state.greensFn->sampleSurface(sampler, sourcePdf);
        }
        float diffusionCurrentPt = pde.diffusion(state.currentPt);
        // if(diffusion == 0){
        //     std::cout<<"Diffusion == 0"<<std::endl;
        //     std::cout<<"Pt: "<< state.currentPt<<std::endl;
        // }
        state.throughput *= std::sqrt(diffusionCurrentPt / diffusion);
        ...
        if (pde.absorptionCoeff > 0.0f && walkSettings.stepsBeforeApplyingTikhonov == state.walkLength) {
            state.greensFn = std::make_shared<YukawaGreensFnBall<DIM>>(pde.bound);
        }
...
template <typename T, size_t DIM>
inline T DeltaTrack<T, DIM>::getTerminalContribution(WalkCompletionCode code,
                                                      const PDEVar<T, DIM>& pde,
                                                      const WalkSettings& walkSettings,
                                                      WalkState<T, DIM>& state) const
{
    if (code == WalkCompletionCode::ReachedAbsorbingBoundary &&
        ...
        bool returnBoundaryNormalAlignedValue = walkSettings.solveDoubleSided &&
                                                signedDistance > 0.0f;
        //std::cout<<"Boundary contribution: "<<pde.dirichlet(state.currentPt, returnBoundaryNormalAlignedValue)<<std::endl;
...
template <typename T, size_t DIM>
inline void DeltaTrack<T, DIM>::estimateSolution(const PDEVar<T, DIM>& pde,
                                                  const WalkSettings& walkSettings,
                                                  int nWalks, SamplePoint<T, DIM>& samplePt) const
{
    ...
    if (samplePt.type == SampleType::OnAbsorbingBoundary) {
        ...
        //std::cout<<"Running Walk on Stars here: Single Walk"<<std::endl;
        nWalks = 1;
    }
    ...
    for (int w = 0; w < nWalks; w++) {
        ...
        while (!stateQueue.empty()) {
            ...
            //std::cout<<"About to start the walk"<<std::endl;
            ...
            //std::cout<<"Terminated Walk"<<std::endl;
            ...
            if (code == WalkCompletionCode::ReachedAbsorbingBoundary ||
                ...
                //std::cout<<"Terminated Walk Succesfully"<<std::endl;
                //std::cout<<"Terminal Condition"<<terminalContribution<<std::endl;
                //std::cout<<"Throughput"<<state.throughput<<std::endl;
...
template <typename T, size_t DIM>
inline void DeltaTrack<T, DIM>::estimateSolutionAndGradient(const PDEVar<T, DIM>& pde,
                                                             const WalkSettings& walkSettings,
                                                             int nWalks, SamplePoint<T, DIM>& samplePt) const
{
