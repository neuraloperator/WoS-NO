...
public:
    ...
    void setSolutionEstimateOnce(const T& estimate) {
        // this is used to set the solution estimate once, e.g., for the first step
        // where the solution is known and does not need to be estimated
    ...
    void setScaleFactor(const T& factor){
        scaleFactor = factor;
    }
...
protected:
    ...
    T scaleFactor; // used to scale the solution and gradient estimates for Spatially varying PDEs;
...
struct SamplePoint {
    ...
    T transformedSource, transformedBoundary, transformedSolution;
    T diffusion, transformedAbsorption;
