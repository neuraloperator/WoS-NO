...
namespace zombie {
    ...
    inline float sampleUniform(float a=0.0f, float b=1.0f)
{
...
inline float sampleNormal(float mean=0.0f, float stddev=1.0f)
{
	thread_local std::mt19937 generator(std::random_device{}());
...
template <int DIM>
inline Vector<DIM> sampleUniformVector(float a=0.0f, float b=1.0f)
{
	Vector<DIM> v;
	for (int i = 0; i < DIM; i++) {
		v(i) = sampleUniform(a, b);
	}
...
template <int DIM>
inline Vector<DIM> sampleNormalVector(float mean=0.0f, float stddev=1.0f)
{
	Vector<DIM> v;
	for (int i = 0; i < DIM; i++) {
		v(i) = sampleNormal(mean, stddev);
	}
...
public:
    ...
    // members
    Vector<DIM> c, yVol, ySurf; // ball center
    float R; // ball radius
    float rClamp;
