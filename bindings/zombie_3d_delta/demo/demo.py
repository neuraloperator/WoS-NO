...
def create_grid_values(output_config, distance_info, values):
    grid_res = output_config["gridRes"]
    boundary_dist_mask = (
        output_config["boundaryDistanceMask"]
        if "boundaryDistanceMask" in output_config
        else 0.0
    )
...
def save_image_buffer(output_config, image_file, image_buffer):
    ...
    colormap = output_config["colormap"] if "colormap" in output_config else "turbo"
    colormap_min_val = (
        output_config["colormapMinVal"] if "colormapMinVal" in output_config else 0.0
    )
    colormap_max_val = (
        output_config["colormapMaxVal"] if "colormapMaxVal" in output_config else 1.0
    )
    save_colormapped = (
        output_config["saveColormapped"] if "saveColormapped" in output_config else True
    )
    ...
    if save_colormapped:
        image = np.clip(
            (image_buffer - colormap_min_val) / (colormap_max_val - colormap_min_val),
            0.0,
            1.0,
        )
        cmap = plt.get_cmap(colormap)
        colormapped_image = cmap(image, bytes=True)
        colormapped_image = np.clip(
            colormapped_image[:, :, :3].astype(np.uint8), 0, 255
        )
...
def setup_pde(model_problem_config, bounding_box):
    # load the model problem configuration
    source_value_buffer, source_value_shape = load_image_buffer(
        model_problem_config["sourceValue"]
    )
    absorbing_boundary_value_buffer, absorbing_boundary_value_shape = load_image_buffer(
        model_problem_config["absorbingBoundaryValue"]
    )
    ...
    is_reflecting_boundary_buffer, is_reflecting_boundary_shape = load_image_buffer(
        model_problem_config["isReflectingBoundary"]
    )
    robin_coeff = (
        model_problem_config["robinCoeff"]
        if "robinCoeff" in model_problem_config
        else 0.0
    )
    absorption_coeff = (
        model_problem_config["absorptionCoeff"]
        if "absorptionCoeff" in model_problem_config
        else 0.0
    )
    ...
    pde.robin = zombie.utils.get_dense_grid_robin_callback_float_2d(
        ...
        reflecting_boundary_value_shape,
        domain_min,
        domain_max,
    )
    pde.has_reflecting_boundary_conditions = (
        zombie.utils.get_dense_grid_indicator_callback_2d(
            is_reflecting_boundary_buffer,
            is_reflecting_boundary_shape,
            domain_min,
            domain_max,
        )
    )
    pde.robin_coeff = zombie.core.get_constant_robin_coefficient_callback_2d(
        robin_coeff
    )
...
def partition_boundary_mesh(has_reflecting_boundary_conditions, positions, indices):
    ...
    zombie.utils.partition_boundary_mesh_2d(
        has_reflecting_boundary_conditions,
        positions,
        indices,
        absorbing_boundary_positions,
        absorbing_boundary_indices,
        reflecting_boundary_positions,
...
def populate_geometric_queries(
    model_problem_config,
    bounding_box,
    absorbing_boundary_positions,
    absorbing_boundary_indices,
    reflecting_boundary_positions,
    ...
    are_robin_conditions_pure_neumann,
    solve_double_sided,
):
    # load the model problem configuration
    domain_is_watertight = (
        model_problem_config["domainIsWatertight"]
        if "domainIsWatertight" in model_problem_config
        else True
    )
    use_sdf_for_absorbing_boundary = (
        model_problem_config["useSdfForAbsorbingBoundary"]
        if "useSdfForAbsorbingBoundary" in model_problem_config
        else False
    )
    sdf_grid_resolution = (
        model_problem_config["sdfGridResolution"]
        if "sdfGridResolution" in model_problem_config
        else 128
    )
    ...
    geometric_queries = zombie.core.geometric_queries_2d(
        domain_is_watertight, domain_min, domain_max
    )
    ...
    dirichlet_boundary_handler.build_acceleration_structure(
        absorbing_boundary_positions, absorbing_boundary_indices
    )
    zombie.utils.populate_geometric_queries_for_dirichlet_boundary_2d(
        dirichlet_boundary_handler, geometric_queries
    )
    ...
    if not solve_double_sided and use_sdf_for_absorbing_boundary:
        ...
        sdf_grid_for_dirichlet_boundary = zombie.utils.sdf_grid_2d(
            domain_min, domain_max
        )
        sdf_grid_shape = np.array(
            [sdf_grid_resolution, sdf_grid_resolution], dtype=np.int32
        )
        zombie.utils.populate_sdf_grid_2d(
            dirichlet_boundary_handler, sdf_grid_for_dirichlet_boundary, sdf_grid_shape
        )
        zombie.utils.populate_geometric_queries_for_dirichlet_boundary_2d(
            sdf_grid_for_dirichlet_boundary, geometric_queries
        )

    # use a reflecting boundary handler to populate geometric queries for the reflecting boundary
    ignore_candidate_silhouette = zombie.utils.get_ignore_candidate_silhouette_callback(
        solve_double_sided
    )
    ...
    if are_robin_conditions_pure_neumann:
        ...
        neumann_boundary_handler.build_acceleration_structure(
            reflecting_boundary_positions,
            reflecting_boundary_indices,
            ignore_candidate_silhouette,
        )
        zombie.utils.populate_geometric_queries_for_neumann_boundary_2d(
            neumann_boundary_handler, branch_traversal_weight, geometric_queries
        )

        return (
            geometric_queries,
            sdf_grid_for_dirichlet_boundary,
            dirichlet_boundary_handler,
            neumann_boundary_handler,
        )

    else:
        robin_boundary_handler = zombie.utils.fcpw_robin_boundary_handler_2d()
        robin_boundary_handler.build_acceleration_structure(
            reflecting_boundary_positions,
            reflecting_boundary_indices,
            ignore_candidate_silhouette,
            min_robin_coeff_values,
        ...
        zombie.utils.populate_geometric_queries_for_robin_boundary_2d(
            robin_boundary_handler, branch_traversal_weight, geometric_queries
        )
...
def compute_distance_info(solve_locations, geometric_queries, solve_double_sided):
    ...
    for i in range(len(solve_locations)):
        ...
        dist_to_absorbing_boundary = (
            geometric_queries.compute_dist_to_absorbing_boundary(pt, False)
        )
        dist_to_reflecting_boundary = (
            geometric_queries.compute_dist_to_reflecting_boundary(pt, False)
        )

        distance_info[i] = (
            in_valid_solve_region,
            dist_to_absorbing_boundary,
            dist_to_reflecting_boundary,
        )
...
def create_sample_points(solve_locations, distance_info):
    ...
    for i in range(len(solve_locations)):
        ...
        estimation_quantity = (
            zombie.solvers.estimation_quantity.solution
            if in_valid_solve_region
        ...
        sample_points[i] = zombie.solvers.sample_point_float_2d(
            pt,
            normal,
            sample_type,
            estimation_quantity,
...
def run_walk_on_stars(
    solver_config, sample_pts, geometric_queries, pde, solve_double_sided
):
    # load config settings
    epsilon_shell_for_absorbing_boundary = (
        solver_config["epsilonShellForAbsorbingBoundary"]
        if "epsilonShellForAbsorbingBoundary" in solver_config
        else 1e-3
    )
    epsilon_shell_for_reflecting_boundary = (
        solver_config["epsilonShellForReflectingBoundary"]
        if "epsilonShellForReflectingBoundary" in solver_config
        else 1e-3
    )
    silhouette_precision = (
        solver_config["silhouettePrecision"]
        if "silhouettePrecision" in solver_config
        else 1e-3
    )
    russian_roulette_threshold = (
        solver_config["russianRouletteThreshold"]
        if "russianRouletteThreshold" in solver_config
        else 0.0
    )
    splitting_threshold = (
        solver_config["splittingThreshold"]
        if "splittingThreshold" in solver_config
        else np.inf
    )

    n_walks = solver_config["nWalks"] if "nWalks" in solver_config else 128
    max_walk_length = (
        solver_config["maxWalkLength"] if "maxWalkLength" in solver_config else 1024
    )
    steps_before_applying_tikhonov = (
        solver_config["stepsBeforeApplyingTikhonov"]
        if "stepsBeforeApplyingTikhonov" in solver_config
        else 0
    )
    steps_before_using_maximal_spheres = (
        solver_config["stepsBeforeUsingMaximalSpheres"]
        if "stepsBeforeUsingMaximalSpheres" in solver_config
        else max_walk_length
    )

    disable_gradient_control_variates = (
        solver_config["disableGradientControlVariates"]
        if "disableGradientControlVariates" in solver_config
        else False
    )
    disable_gradient_antithetic_variates = (
        solver_config["disableGradientAntitheticVariates"]
        if "disableGradientAntitheticVariates" in solver_config
        else False
    )
    use_cosine_sampling_for_directional_derivatives = (
        solver_config["useCosineSamplingForDirectionalDerivatives"]
        if "useCosineSamplingForDirectionalDerivatives" in solver_config
        else False
    )
    ignore_absorbing_boundary_contribution = (
        solver_config["ignoreAbsorbingBoundaryContribution"]
        if "ignoreAbsorbingBoundaryContribution" in solver_config
        else False
    )
    ignore_reflecting_boundary_contribution = (
        solver_config["ignoreReflectingBoundaryContribution"]
        if "ignoreReflectingBoundaryContribution" in solver_config
        else False
    )
    ignore_source_contribution = (
        solver_config["ignoreSourceContribution"]
        if "ignoreSourceContribution" in solver_config
        else False
    )
    print_logs = solver_config["printLogs"] if "printLogs" in solver_config else False
    run_single_threaded = (
        solver_config["runSingleThreaded"]
        if "runSingleThreaded" in solver_config
        else False
    )
    ...
    walk_settings = zombie.solvers.walk_settings(
        epsilon_shell_for_absorbing_boundary,
        epsilon_shell_for_reflecting_boundary,
        silhouette_precision,
        russian_roulette_threshold,
...
def create_boundary_sampler(
    ...
    indices,
    geometric_queries,
    normal_offset_for_boundary,
    solve_double_sided,
):
    boundary_sampler = (
        zombie.samplers.create_uniform_line_segment_boundary_sampler_float_2d(
...
def create_bvc_evaluation_points(solve_locations, distance_info):
    ...
    for i in range(len(solve_locations)):
        ...
        evaluation_points[i] = zombie.solvers.bvc_evaluation_point_float_2d(
            pt,
            normal,
            sample_type,
            dist_to_absorbing_boundary,
...
def run_boundary_value_caching(
    solver_config,
    evaluation_pts,
    absorbing_boundary_positions,
    absorbing_boundary_indices,
    ...
    n_walks_for_cached_solution_estimates = (
        solver_config["nWalksForCachedSolutionEstimates"]
        if "nWalksForCachedSolutionEstimates" in solver_config
        else 128
    )
    n_walks_for_cached_gradient_estimates = (
        solver_config["nWalksForCachedGradientEstimates"]
        if "nWalksForCachedGradientEstimates" in solver_config
        else 640
    )
    absorbing_boundary_cache_size = (
        solver_config["absorbingBoundaryCacheSize"]
        if "absorbingBoundaryCacheSize" in solver_config
        else 1024
    )
    reflecting_boundary_cache_size = (
        solver_config["reflectingBoundaryCacheSize"]
        if "reflectingBoundaryCacheSize" in solver_config
        else 1024
    )
    domain_cache_size = (
        solver_config["domainCacheSize"] if "domainCacheSize" in solver_config else 1024
    )

    use_finite_differences_for_boundary_derivatives = (
        solver_config["useFiniteDifferencesForBoundaryDerivatives"]
        if "useFiniteDifferencesForBoundaryDerivatives" in solver_config
        else False
    )

    robin_coeff_cutoff_for_normal_derivative = (
        solver_config["robinCoeffCutoffForNormalDerivative"]
        if "robinCoeffCutoffForNormalDerivative" in solver_config
        else np.inf
    )
    normal_offset_for_absorbing_boundary = (
        solver_config["normalOffsetForAbsorbingBoundary"]
        if "normalOffsetForAbsorbingBoundary" in solver_config
        else 5.0 * epsilon_shell_for_absorbing_boundary
    )
    normal_offset_for_reflecting_boundary = (
        solver_config["normalOffsetForReflectingBoundary"]
        if "normalOffsetForReflectingBoundary" in solver_config
        else 0.0
    )
    radius_clamp_for_kernels = (
        solver_config["radiusClampForKernels"]
        if "radiusClampForKernels" in solver_config
        else 0.0
    )
    regularization_for_kernels = (
        solver_config["regularizationForKernels"]
        if "regularizationForKernels" in solver_config
        else 0.0
    )
    ...
    total_work = (
        2 * (absorbing_boundary_cache_size + reflecting_boundary_cache_size)
        + domain_cache_size
    )
    ...
    boundary_value_caching.generate_samples(
        absorbing_boundary_cache_size,
        reflecting_boundary_cache_size,
        domain_cache_size,
        normal_offset_for_absorbing_boundary,
    ...
    boundary_value_caching.compute_sample_estimates(
        pde,
        walk_settings,
        n_walks_for_cached_solution_estimates,
        n_walks_for_cached_gradient_estimates,
    ...
    boundary_value_caching.estimate_solution_near_boundary(
        pde,
        walk_settings,
        normal_offset_for_absorbing_boundary,
        normal_offset_for_reflecting_boundary,
...
def create_rws_evaluation_points(solve_locations, distance_info):
    ...
    for i in range(len(solve_locations)):
        ...
        evaluation_points[i] = zombie.solvers.rws_evaluation_point_float_2d(
            pt,
            normal,
            sample_type,
            dist_to_absorbing_boundary,
...
def run_reverse_walk_on_stars(
    ...
    absorbing_boundary_sample_count = (
        solver_config["absorbingBoundarySampleCount"]
        if "absorbingBoundarySampleCount" in solver_config
        else 1024
    )
    reflecting_boundary_sample_count = (
        solver_config["reflectingBoundarySampleCount"]
        if "reflectingBoundarySampleCount" in solver_config
        else 1024
    )
    domain_sample_count = (
        solver_config["domainSampleCount"]
        if "domainSampleCount" in solver_config
        else 1024
    )
    ...
    reverse_walk_on_stars = zombie.solvers.reverse_walk_on_stars_float_2d(
        geometric_queries,
        absorbing_boundary_sampler,
        reflecting_boundary_sampler,
        domain_sampler,
    ...
    # generate boundary and domain samples
    reverse_walk_on_stars.generate_samples(
        absorbing_boundary_sample_count,
        reflecting_boundary_sample_count,
        domain_sample_count,
        normal_offset_for_absorbing_boundary,
    ...
    return [
        reverse_walk_on_stars.get_absorbing_boundary_sample_count(False),
...
def run_solver(
    ...
    if solver_type == "wost":
        ...
        sample_counts = run_reverse_walk_on_stars(
            solver_config,
            evaluation_pts,
            absorbing_boundary_positions,
            absorbing_boundary_indices,
...
if __name__ == "__main__":
    ...
    try:
        ...
        (
            ...
            pde.has_reflecting_boundary_conditions, positions, indices
        )
        ...
        min_robin_coeff_values = zombie.float_list(
            [abs(robin_coeff)] * len(reflecting_boundary_indices)
        )
        max_robin_coeff_values = zombie.float_list(
            [abs(robin_coeff)] * len(reflecting_boundary_indices)
        )

        # populate the geometric queries for the absorbing and reflecting boundary
        solve_double_sided = (
            model_problem_config["solveDoubleSided"]
            if "solveDoubleSided" in model_problem_config
            else False
        )
        (
            geometric_queries,
            sdf_grid_for_absorbing_boundary,
            absorbing_boundary_handler,
            reflecting_boundary_handler,
        ) = populate_geometric_queries(
            model_problem_config,
            bounding_box,
            absorbing_boundary_positions,
            absorbing_boundary_indices,
            ...
            min_robin_coeff_values,
            max_robin_coeff_values,
            pde.are_robin_conditions_pure_neumann,
            solve_double_sided,
        )
        ...
        distance_info = compute_distance_info(
            solve_locations, geometric_queries, solve_double_sided
        )
        ...
        solution_file = (
            output_config["solutionFile"]
            if "solutionFile" in output_config
            else "solution.png"
        )
