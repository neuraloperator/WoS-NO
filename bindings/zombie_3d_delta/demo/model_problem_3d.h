...
class ModelProblem3DVar {
public:
    // constructor
    ModelProblem3DVar<T, DIM>(const json& config, std::string directoryPath, std::vector<float>absorption, std::vector<float>diffusion, std::vector<float>dirichlet, std::vector<float>general);
    using Int32   = int32_t;
...
// Implementation
template <typename T, size_t DIM>
ModelProblem3DVar<T, DIM>::ModelProblem3DVar(const json& config, std::string directoryPath, std::vector<float>absorption, std::vector<float>diffusion, std::vector<float>dirichlet, std::vector<float>general):
...
template <typename T, size_t DIM>
void ModelProblem3DVar<T, DIM>::loadOBJ(const std::string& filename, bool normalize, bool flipOrientation)
{
...
template <typename T, size_t DIM>
void ModelProblem3DVar<T, DIM>::setupPDE()
{
    ...
    pde.absorptionFunc = [this, absorptionMin, absorptionMax](const VectorD& x) -> float {
        return absorptionMin + (absorptionMax - absorptionMin) * (1.0f + 0.5f*std::sin(2.0f*M_PI*x(0))*std::cos(0.5f*M_PI*x(1)));
    };
    pde.diffusionFunc = [this, diffusionFreq](const VectorD& x, 
    ...
    pde.source = [this, dirichletFreq](const VectorD& x) -> float {
    ...
    pde.dirichlet = [this, dirichletFreq](const VectorD& x, bool _) -> float {
    ...
    pde.bound = compute_bound(pde, {-1.0, -1.0, -1.0},{0.999999f, 1.000000f, 1.000001f});
...
template <typename T, size_t DIM>
void ModelProblem3DVar<T, DIM>::partitionBoundaryMesh()
{
...
template <typename T, size_t DIM>
void ModelProblem3DVar<T, DIM>::populateGeometricQueries()
{
