...
{
    //std::cout<<"Sample Point Size: "<<solveLocations.size()<<std::endl;
    for (int i = 0; i < (int)solveLocations.size(); i++) {
        zombie::Vector<DIM> pt = solveLocations[i];
...
template <typename T, size_t DIM>
void runDeltaTrack(const json& solverConfig,
                    const zombie::GeometricQueries<DIM>& queries,
                    const zombie::PDEVar<T, DIM>& pde,
                    bool solveDoubleSided,
                    std::vector<zombie::SamplePoint<T, DIM>>& samplePts)
{
    ...
    const bool ignoreReflectingBoundaryContribution = getOptional<bool>(solverConfig, "ignoreReflectingBoundaryContribution", true);
    const bool ignoreSourceContribution = getOptional<bool>(solverConfig, "ignoreSourceContribution", false);
    const bool printLogs = getOptional<bool>(solverConfig, "printLogs", true);
    const bool runSingleThreaded = getOptional<bool>(solverConfig, "runSingleThreaded", false);
    //std::cout<<"Sample Points Size: "<<samplePts.size()<<std::endl;
    ...
    zombie::DeltaTrack<T, DIM> deltaTrack(queries);
    deltaTrack.solve(pde, walkSettings, nWalksVector, samplePts, runSingleThreaded, reportProgress);
    pb.finish();
...
{
    std::cout<<"Sample Pts Size: "<<samplePts.size()<<std::endl;
...
template <typename T, size_t DIM>
void createBvcDtEvaluationPoints(const std::vector<zombie::Vector<DIM>>& solveLocations,
                               const std::vector<DistanceInfo>& distanceInfo,
                               std::vector<zombie::bvc_dt::EvaluationPoint<T, DIM>>& evalPts)
{
    for (int i = 0; i < (int)solveLocations.size(); i++) {
        zombie::Vector<DIM> pt = solveLocations[i];
        zombie::Vector<DIM> normal = zombie::Vector<DIM>::Zero();
        ...
        evalPts.emplace_back(zombie::bvc_dt::EvaluationPoint<T, DIM>(pt, normal, sampleType,
                                                                  distToAbsorbingBoundary,
                                                                  distToReflectingBoundary));
    }
}
...
template <typename T, size_t DIM>
void runBoundaryValueCachingDeltaTrack(const json& solverConfig,
                             const std::vector<zombie::Vector<DIM>>& absorbingBoundaryPositions,
                             const std::vector<zombie::Vectori<DIM>>& absorbingBoundaryIndices,
                             const std::vector<zombie::Vector<DIM>>& reflectingBoundaryPositions,
                             const std::vector<zombie::Vectori<DIM>>& reflectingBoundaryIndices,
                             ...
                             std::vector<zombie::bvc_dt::EvaluationPoint<T, DIM>>& evalPts)
{
    ...
    std::shared_ptr<zombie::BoundarySampler<T, DIM>> absorbingBoundarySampler = createBoundarySampler<T>(
        absorbingBoundaryPositions, absorbingBoundaryIndices, queries);
    absorbingBoundarySampler->initialize(normalOffsetForAbsorbingBoundary, solveDoubleSided);
    std::cout<<"Absorbing Boundary Sampler Initialized"<<" Absorbing Boundary Indices " << absorbingBoundaryIndices.size()<<std::endl;

    std::shared_ptr<zombie::BoundarySampler<T, DIM>> reflectingBoundarySampler = createBoundarySampler<T>(
        reflectingBoundaryPositions, reflectingBoundaryIndices, queries);
    reflectingBoundarySampler->initialize(normalOffsetForReflectingBoundary, solveDoubleSided);
    std::cout<<"Reflecting Boundary Sampler Initialized"<< " Reflecting Boundary Indices " << reflectingBoundaryIndices.size()<<std::endl;
    ...
    zombie::bvc_dt::BoundaryValueCachingSolver<T, DIM> boundaryValueCaching(
        queries, absorbingBoundarySampler, reflectingBoundarySampler, domainSampler);
...
template <typename T, size_t DIM>
void getSolutionDt(const std::vector<zombie::bvc_dt::EvaluationPoint<T, DIM>>& evalPts,
                 std::vector<T>& solution)
{
    solution.resize(evalPts.size(), T(0.0f));
    for (int i = 0; i < (int)evalPts.size(); i++) {
...
void runSolver(const std::string& solverType, const json& config,
               ...
               const zombie::GeometricQueries<DIM>& queries,
               const zombie::PDE<T, DIM>& pde, 
               const zombie::PDEVar<T, DIM>& pdeVar, bool solveDoubleSided,
               const std::vector<zombie::Vector<DIM>>& solveLocations,
               const std::vector<DistanceInfo>& distanceInfo,
               std::vector<T>& solution, std::vector<std::vector<T>>& gradient)
{
    std::cout<<"Running Solver: "<<solverType<<std::endl;
    if (solverType == "wost") {
        ...
        // for (int i = 0; i< (int)samplePts.size(); i++){
        //     std::cout<<"Gradient: "<<*samplePts[i].statistics.getEstimatedGradient()<<std::endl;
        //     // for (auto& vec: samplePts[i].statistics.getEstimatedGradient()){
        //     //     gradient[i].emplace_back(vec.data());
        //     // }
        // }

    } else if (solverType == "dtrack") {
        std::cout<<"Inside DTrack"<<std::endl;
        std::vector<zombie::SamplePoint<T, DIM>> samplePts;
        createSamplePoints<T, DIM>(solveLocations, distanceInfo, samplePts);
        std::cout<<"Sample Points Size: "<<samplePts.size()<<std::endl;
        // run delta track
        runDeltaTrack<T, DIM>(config, queries, pdeVar, solveDoubleSided, samplePts);

        // extract solution from sample points
        getSolution<T, DIM>(samplePts, solution);
    
    
    } else if (solverType == "bvc_dt"){
        // create evaluation points to estimate solution at
        std::vector<zombie::bvc_dt::EvaluationPoint<T, DIM>> evalPts;
        createBvcDtEvaluationPoints<T, DIM>(solveLocations, distanceInfo, evalPts);

        // run boundary value caching
        runBoundaryValueCachingDeltaTrack<T, DIM>(config, absorbingBoundaryPositions, absorbingBoundaryIndices,
                                        reflectingBoundaryPositions, reflectingBoundaryIndices,
                                        queries, pdeVar, solveDoubleSided, evalPts);

        // extract solution from evaluation points
        getSolutionDt<T, DIM>(evalPts, solution);
    } 
...
template <typename T, size_t DIM>
std::tuple<std::vector<std::vector<float>>, std::vector<T>, std::vector<std::vector<T>>> runZombie(const ModelProblem<T, DIM>& scene, const json& solverConfig, const json& outputConfig, std::vector<std::vector<float>> sample_points){
    using Int32   = int32_t;
    using Vectorint = Eigen::Matrix<int, DIM, 1>;;
    using VectorD = Eigen::Matrix<T, DIM, 1>;
    using VectorDa = Eigen::Array<T, DIM, 1>;
    const std::string solverType = getOptional<std::string>(solverConfig, "solverType", "wost");
    const std::string zombieDirectoryPath = "../";
    const std::vector<Vectorint>& absorbingBoundaryIndices = scene.absorbingBoundaryIndices;
    const std::vector<Vectorint>& reflectingBoundaryIndices = scene.reflectingBoundaryIndices;
    const std::vector<VectorD>& absorbingBoundaryPositions = scene.absorbingBoundaryPositions;
    const std::vector<VectorD>& reflectingBoundaryPositions = scene.reflectingBoundaryPositions;
    const std::pair<VectorD, VectorD>& boundingBox = scene.boundingBox;
    const zombie::GeometricQueries<DIM>& queries = scene.queries;
    const zombie::PDE<T, DIM>& pde = scene.pde;
    const zombie::PDEVar<T, DIM>& pdeVar = scene.pdeVar;
    bool solveDoubleSided = scene.solveDoubleSided;
    std::vector<VectorD> solveLocations;
    std::vector<DistanceInfo> distanceInfo;
    std::cout<<"Inside Zombie Sample Points Size: "<<sample_points.size()<<std::endl;
    //solveLocations.resize(sample_points.size());
    for (auto & vec: sample_points){

        Eigen::Matrix<float, DIM, 1> vect;
        vect = Eigen::Map<const Eigen::Matrix<float, DIM, 1>>(vec.data());
        
        solveLocations.emplace_back(vect);
    }
    std::cout<<"Solve Locations Size: "<<solveLocations.size()<<std::endl;
    
    computeDistanceInfo<DIM>(solveLocations, queries, solveDoubleSided, distanceInfo);
    std::cout<<"Computed Distance Info Size: "<<std::endl;
    std::vector<T> solution;
    std::vector<std::vector<T>> gradient;
    runSolver<T, DIM>(solverType, solverConfig, absorbingBoundaryPositions, absorbingBoundaryIndices,
                                reflectingBoundaryPositions, reflectingBoundaryIndices, queries, pde, pdeVar,
                                solveDoubleSided, solveLocations, distanceInfo, solution, gradient);
...
PYBIND11_MODULE(zombie, m) {
    m.doc() = "pybind11 WoSt"; // optional module docstring
    m.def("zombie2d", &runZombie<float,2>);
    m.def("zombie3d", &runZombie<float,3>);
	
    py::class_<ModelProblem<float, 2>>(m, "Scene2D")
		.def(py::init<json &, std::string &,std::vector<std::vector<std::vector<float>>> &>());
    
    py::class_<ModelProblem<float, 3>>(m, "Scene3D")
        .def(py::init<json &, std::string &,std::vector<std::vector<std::vector<float>>> &>())
        .def(py::init<json &, std::string &, std::vector<float> &, std::vector<float> &, std::vector<float> &, std::vector<float> &>());
}
