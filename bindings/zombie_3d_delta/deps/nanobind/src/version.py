...
def write_version(root, semver):
    ...
    with open(os.path.join(root, "include/nanobind/nanobind.h"), "r+") as f:
        contents = f.read()
        contents = re.sub(
            r"#define NB_VERSION_MAJOR\s+[0-9]+",
            r"#define NB_VERSION_MAJOR " + str(major),
            contents,
            count=1,
        )
        contents = re.sub(
            r"#define NB_VERSION_MINOR\s+[0-9]+",
            r"#define NB_VERSION_MINOR " + str(minor),
            contents,
            count=1,
        )
        contents = re.sub(
            r"#define NB_VERSION_PATCH\s+[0-9]+",
            r"#define NB_VERSION_PATCH " + str(patch),
            contents,
            count=1,
        )
        contents = re.sub(
            r"#define NB_VERSION_DEV\s+[0-9]+",
    ...
    with open(os.path.join(root, "pyproject.toml"), "r+") as f:
        contents = f.read()
        contents = re.sub(
            r'version\s+=\s+"[^"]+"', r'version = "' + semver + '"', contents, count=1
        )
        f.seek(0)
        f.truncate()
        f.write(contents)
