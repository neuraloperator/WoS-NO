...
def load_fcpw_scene(positions, indices, build_vectorized_cpu_bvh):
    ...
    scene.build(
        aggregate_type, build_vectorized_cpu_bvh, print_stats, reduce_memory_footprint
    )
...
def perform_gpu_closest_point_queries(gpu_scene, query_points):
    ...
    closest_points = np.array(
        [
            np.array([i.p.x, i.p.y, i.p.z], dtype=np.float32, order="C")
...
def gui_callback(scene, query_points, use_gpu):
    ...
    edge_indices = np.array(
        [[i, i + len(query_points)] for i in range(len(query_points))]
    )
...
def main():
    ...
    query_points = np.random.uniform(box_min, box_max, (num_query_points, 3)).astype(
        np.float32
    )
    ...
    if args.use_gpu:
        ...
        scene = load_fcpw_scene(
            positions, indices, False
        )  # NOTE: must build non-vectorized CPU BVH
