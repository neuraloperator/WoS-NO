...
class StaticArraySynthProvider:
    ...
    def get_summary(self):
        if self.is_simple:
            if not self.is_mask:
                result = [
                    str(self.get_child_at_index(i).value) for i in range(self.size)
                ]
            else:
                if self.kmask:
                    # AVX512 mask register
                    result = list(
                        reversed(format(int(self.kmask.unsigned), "0%ib" % self.size))
                    )
                else:
...
class DynamicArraySummaryProvider:
    ...
    def update(self):
        ...
        self.packet_count = self.instance.GetChildMemberWithName(
            "m_packets_allocated"
        ).unsigned
        self.packet_type = (
            self.instance.GetType()
            .GetCanonicalType()
            .GetUnqualifiedType()
        ...
        error = lldb.SBError()
        self.ptr = (
            self.ptr.GetUnsignedInt64(offset=0, error=error)
            if self.ptr.GetByteSize() == 8
    ...
    def get_summary(self):
        ...
        for i in range(self.packet_count):
            value = str(
                self.instance.CreateValueFromAddress(
                    str(i), self.ptr + i * self.packet_size, self.packet_type
                )
            )
            assert value[-1] == "]"
...
def __lldb_init_module(debugger, internal_dict):
    ...
    # Static Enoki arrays
    regexp_1 = (
        r"enoki::(Array|Packet|Complex|Matrix|" "Quaternion|StaticArrayImpl)(Mask)?<.+>"
    )
    ...
    attach(
        enoki_category, DynamicArraySummaryProvider, r"^enoki::DynamicArray(Impl)?<.+>$"
    )
