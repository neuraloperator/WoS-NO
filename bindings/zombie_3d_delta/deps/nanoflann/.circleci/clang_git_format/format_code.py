...
class ClangRepoFormatter(object):
    ...
    def run(self):
        ...
        parser.description = (
            "Apply clang-format to a whole Git repository. "
        ...
        parser.add_argument(
            ...
            required=True,
            help=(
                "Relative path to the root of the git repo that "
        ...
        parser.add_argument(
            ...
            help=(
                "Languages used in the repository. This is used to determine"
                "the files which clang format runs for. Default langs: %s.".format(
                    default_langs
                )
            ),
        )
        parser.add_argument(
            ...
            required=False,
            help=(
                "Custom regular expression to apply to the files that are "
        ...
        parser.add_argument(
            ...
            required=False,
            help=(
                "Sequence of directories. If given clang-format is going to "
        ...
        commands_group.add_argument(
            ...
            nargs=3,
            help=(
                "Reformat CURRENT branch given the <start-1> and"
                " <end-2> commits."
        ...
        commands_group.add_argument(
            ...
            action="store_true",
            help=(
                "Run clang-format against files that may or may not "
        ...
        self.clang_format = ClangFormat(
            parser_args["clang_format"], self._get_build_dir()
        )

        # git repo
        self.git_repo = Repo(
            parser_args["git_repo"],
        ...
        if parser_args["lint"]:
            ...
            logger.fatal(
                "Unexpected error in the parsing of command line " "arguments! Exiting."
            )
    ...
    def _lint_files(self, files):
        ...
        lint_clean = parallel_process(
            [os.path.abspath(f) for f in files], self.clang_format.lint
        )
    ...
    def _format_files(self, files):
        ...
        format_clean = parallel_process(
            [os.path.abspath(f) for f in files], self.clang_format.format_func
        )
    ...
    def reformat_branch(
        self, commit_prior_reformat, commit_after_reformat, main_branch
    ):
        ...
        if len(commit_prior_reformat) < min_commit_id_len:
            raise CommitIDTooShort(commit_prior_reformat, min_commit_id_len)
        if len(commit_after_reformat) < min_commit_id_len:
            raise CommitIDTooShort(commit_after_reformat, min_commit_id_len)
        ...
        if (
            not merge_base[0:min_commit_id_len]
            == commit_prior_reformat[0:min_commit_id_len]
        ):
            raise ValueError(
                "Please **rebase** your work to '%s' and resolve all "
                "conflicts before running this script" % (commit_prior_reformat)
            )
        ...
        if repo.does_branch_exist(new_branch):
            raise ValueError(
                ...
                "Please delete the "
                "branch '%s', or rename the current branch." % (new_branch, new_branch)
            )

        commits = self.get_list_from_lines(
            repo.log(
                ["--reverse", "--pretty=format:%H", "%s..HEAD" % commit_prior_reformat]
            )
        )
        ...
        for commit_hash in commits:
            ...
            # the user's branch
            commit_files = self.get_list_from_lines(repo.diff(["HEAD~", "--name-only"]))
            ...
            for commit_file in commit_files:
                ...
                if not os.path.exists(commit_file):
                    logger.warning(
                        "Skipping file '%s' since it has been "
                        "deleted in commit '%s'" % (commit_file, commit_hash)
                    )
                ...
                if commit_file in files_to_check:
                    ...
                    logger.info(
                        "Skipping file '%s' since it is not a "
