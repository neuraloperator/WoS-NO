...
class WOSPoisson2DSolver(ZombieSolver):
    ...
    def run_solver(self, samples, py_gino_fn=None):
        """
        ...
        # TODO: Check if we need to have variance returned!
        # pts, solution, gradient, variance = zombie_bindings.wost(self.scene, self.solver_config, self.output_config, samples)
        if py_gino_fn is not None:
            print("Using py_gino_fn")
            pts, solution, gradient = zombie_bindings.wost(
                self.scene, self.solver_config, self.output_config, samples, py_gino_fn
            )
        else:
            pts, solution, gradient = zombie_bindings.wost(
        ...
        return pts, solution, gradient

    def solve(self, points, coefs, geometry, py_gino_fn=None):
        ...
        samples, p_arr, grad_arr = self.run_solver(points, py_gino_fn=py_gino_fn)

        return samples, p_arr, grad_arr
